...
// anchored on this rather than on the tail of the log, because most tools print
// a summary, a stack, and an exit code after the useful part.
//
// `pnpm build` is `tsc -b && vite build`, so the first error can come from
// either half. Vite errors start with a keyword; tsc errors start with the file
// path and carry `(line,col): error TS` in the middle, so both forms are here.
const ERROR_HEADING = /^\s*(error|ERROR|Error:|\[vite\]|✘|×|failed to)|\(\d+,\d+\): error TS/
